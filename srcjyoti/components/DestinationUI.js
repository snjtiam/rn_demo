...
import {DestinationComponent} from './UI_components/DestinationComponent';
import { SectionTitle } from './UI_components/DestinationComponent';
import { Section_Package } from './UI_components/DestinationComponent';
// how to render huge amount of arrays as per required order
  ...
  return (
    <ScrollView>
      <TextInput
        ...
        style={{ backgroundColor: 'white', marginHorizontal: 25, marginTop: 2, borderRadius: 15 }}
      />
      ...
      <FlatList
        ...
        data={data}  
        renderItem={({ item }) => (
          <DestinationComponent imgUrl={item.imgUrl} destination={item.destination} country={item.country} onPress={() => {onPressImg(item)}}
          />
        )}
      />
      ...
      data={data2}
      renderItem={({item})=>(
          <Section_Package imgUrl={item.imgUrl} destination={item.destination} description={item.description} Price={item.Price}/>
      )}  
      />
